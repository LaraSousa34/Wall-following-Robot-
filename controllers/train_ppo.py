...
#defining the PPO as the DRL model for training
#model = PPO('MlpPolicy', env, verbose=1, tensorboard_log=tensorboard_log_dir)

#saving the model: creating a zip file
#model.save("ppo_wallfollowing")  # vai criar a pasta no dir de onde estamos a correr

# loading the saved model
#model = PPO.load(r'/Users/larasousa/Desktop/IRI/controllers/Training/PPO/20000', env)  #loading last PPO zip for PPO_2
#model = PPO.load(r'/Users/larasousa/Desktop/IRI/controllers/Training/PPO_2/20000', env) #loading last PPO_2 zip for PPO_3
model = PPO.load(r'/Users/larasousa/Desktop/IRI/controllers/Training/PPO_3/20000', env) #loading last PPO_2 zip for PPO_3
#odel = PPO.load(r'/Users/larasousa/Desktop/IRI/controllers/Training/PPO_4_5/40000', env) #loading last PPO_2 zip for PPO_3
#model = PPO.load(r'/Users/larasousa/Desktop/IRI/controllers/Training/PPO_4_5_2/40000', env) #loading last PPO_2 zip for PPO_3
...
while iters < max_iters:
    ...
    # create Trainig/PPO's folders in the same directory where we have the python files
    # inside that the files are saved

    #model.save(f"{'Training/PPO'}/{timesteps * iters}")
    #model.save(f"{'Training/PPO_2'}/{timesteps * iters}")
    #model.save(f"{'Training/PPO_3'}/{timesteps * iters}")
    model.save(f"{'Training/PPO_4_5'}/{timesteps * iters}")
    #model.save(f"{'Training/PPO_4_5_2'}/{timesteps * iters}")
    #model.save(f"{'Training/PPO_4_5_3'}/{timesteps * iters}")